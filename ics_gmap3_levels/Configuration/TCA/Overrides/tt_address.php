...
$indexPalettes = 0;
foreach ($GLOBALS['TCA']['tt_address']['palettes'] as $index => $value) {
    if ($index > $indexPalettes) {
        $indexPalettes = $index;
    }
}
$indexPalettes++;
\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::addToAllTCAtypes('tt_address', '--div--;Geoloc,--palette--;LLL:EXT:ics_gmap3_levels/Resources/Private/Language/locallang_db.xlf:tx_icsgmap3levels_levels;' . $indexPalettes);
$GLOBALS['TCA']['tt_address']['palettes'][$indexPalettes] = array('showitem' => 'tx_icsgmap3levels_level', 'canNotCollapse' => 1);
