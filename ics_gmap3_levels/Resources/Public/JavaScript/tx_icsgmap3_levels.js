...
(function () {
    ...
    ics.HierarchicalTagList.prototype.init = function (map, exclusivesTags, hiddenTags, defaultTags, viewDefaultTags, separator, checkOnParent, viewLinkSelectAll, lang) {
        ...
        if (!map.levelsKmlFinish) {
            ...
            window.setTimeout(function () {
                ...
                funcObj.init(map, exclusivesTags, hiddenTags, defaultTags, viewDefaultTags, separator, checkOnParent, viewLinkSelectAll, lang);
            }, 1000);
        } else {
            map.levelsKmlArray = [];
            jQuery.each(map.levelsKml, function (tag, kml) {
                if (tag && kml) {
                    if (jQuery.inArray(tag, map.htl.markersTags) < 0) {
    ...
    ics.HierarchicalTagList.prototype.click_ = function (element, map) {
        ...
        if (ics.Map.prototype.elementsClicked === undefined) {
            ics.Map.prototype.elementsClicked = [];
        }
        if (!element.checked) {
            eClicked = [];
        ...
        if (kml) {
            ...
            ics.Map.prototype.createKmlLayer(kml, !!element.checked, 'levels_' + element.value.replace(/[ #-,]/gi, "_"));
        }
    }
})();
